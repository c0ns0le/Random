...
if [ -z "$beoservpid" -o -z "$bpmasterpid" -o -z "$recvstatspid" ];then
  ...
  logger -p crit "URGENT ALERT CALL TIER II - The daemon goldd is not running.  It was not found in the process table."
  exit 1
  ...
  logger -p crit "URGENT ALERT CALL TIER II - The daemon goldd appears to be running but is not functional.  It was unable to print the project list."
  exit 1
else
