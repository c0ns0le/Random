...
while [ "$node" -lt "$num_nodes" ];do
  ...
  #MAC of eth0
  bpsh n1 /sbin/ifconfig eth0 | awk '/HWaddr/ {print $5;exit}' | tr '\n' ','>>"$output_file"
  #MAC of eth1
  bpsh n1 /sbin/ifconfig eth1 | awk '/HWaddr/ {print $5;exit}' | tr '\n' ','>>"$output_file"
  #Type of CPU
  bpsh n1 awk -F': ' '/model name/ {print $2;exit}' /proc/cpuinfo | tr '\n' ','>>"$output_file"
  #Number of CPU cores
  bpsh n1 egrep -c '^processor' /proc/cpuinfo | tr '\n' ','>>"$output_file"
  #GPU found?
  bpsh n1 lsmod | egrep '^nvidia' >/dev/null
  if [ "$?" = "0" ];then
  ...
  #Infiniband found?
  bpsh n1 ifconfig -a | egrep '^ib' >/dev/null
  if [ "$?" = "0" ];then
    printf "1,">>"$output_file"
  else
    printf "0,">>"$output_file"
  ...
  if bpsh n1 df -kP | grep '/scratch'>/dev/null;then #If there is a scratch disk found...
    bpsh n1 df -kP | awk '/\/scratch/ {print $4/1024/1024;exit}' | tr '\n' ','>>"$output_file"
  else
    printf ",">>"$output_file"
  fi
  #Amount of RAM
  bpsh n1 awk '/^MemTotal/ {print $2;exit}' /proc/meminfo | tr '\n' ','>>"$output_file"
  ...
  printf ",">>"$output_file"
  #Ethernet IP
  eth_ip=$(bpsh n1 ifconfig eth0 | awk '/inet addr/ {print $2}' | cut -d':' -f2)
  echo $eth_ip | tr '\n' ','>>"$output_file"
  #IB IP
