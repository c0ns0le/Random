...
for my $user (keys %user_numfiles) {
  ...
  print "User: $user\n";
  print "Number of files: $user_numfiles{$user}\n";
...
for my $group (keys %group_numfiles) {
  ...
  print "Group: $group\n";
  print "Number of files: $group_numfiles{$group}\n";
...
print "Average number of files per user: " . sprintf("%.2f", $total_num_files / keys(%user_numfiles)) . "\n";
print "Average number of files per group: " . sprintf("%.2f", $total_num_files / keys(%group_numfiles)) . "\n";
print "\nLargest users:\n";
my $user_loop_count;
foreach my $user (sort {$user_datausage{$b} <=> $user_datausage{$a}} keys %user_datausage) {
  print "$user " . sprintf("%.2f", $user_datausage{$user} / 1024 / 1024 / 1024) . " GB \n";
  $user_loop_count++;
...
foreach my $group (sort {$group_datausage{$b} <=> $group_datausage{$a}} keys %group_datausage) {
  print "$group " . sprintf("%.2f", $group_datausage{$group} / 1024 / 1024 / 1024) . " GB \n";
  $group_loop_count++;
